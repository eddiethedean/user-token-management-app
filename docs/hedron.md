...
| 0.65.0 presentation contract | The app's `data-mover` brand is authored with Hedron `Color`, `ThemeBuilder`, validated `ThemeSpec`, accessibility modes, theme variants, typed recipe families, named control/surface/data/status/content recipes, and scoped auth/workspace recipe defaults. |
| 0.65.0 release train | Runtime and Posit integration are pinned to `>=0.65.0,<0.66`; the 0.65.0 pass is verified against route, interaction, security, and deployment boundaries. |
...
| 0.65.0 styling contract | The app validates `DATA_MOVER_THEME` through Hedron's CSS/design-token export and 0.65.0 presentation contract, using typed Brand mark controls, `AmbientBackdrop`, presentation scales, native control/data tokens, the component bundle, canonical selection/link tokens, and compatibility aliases for the default stylesheet. |
...
## 0.65.0 status update

Data Mover is pinned to Hedron 0.65.0 and the compatible 0.65 train. The app deliberately keeps its existing
...
The 0.65.0 presentation layer is active: the Data Mover brand compiler starts from Hedron's bundled
...
Run `make hedron-security-check` after installing the 0.65.0 environment.
...
The product stylesheet contains product art direction only. Hedron 0.65.0 now owns canonical theme
...
1. Update the bounded Hedron 0.65.0 dependency and rebuild the virtual environment.
