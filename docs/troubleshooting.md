...
| 404 on a rewritten `/proxy/8000/s/…/p/…/login` URL | A compatibility proxy added its own prefix to the app's path redirect | Open the URL printed by `serve` (the `/s/…/p/…` session URL), or remove the compatibility proxy from the path; do not hand-edit the URL |
