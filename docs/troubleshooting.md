...
| `Could not proxy POST request to /proxy/8000/login: connect ECONNREFUSED` while `make demo` says port 8765 | The browser URL or form mount belongs to port 8000, but the demo is listening on 8765 | Pull the current launcher, restart `make demo`, and open the newly printed Workbench URL. It discovers and configures the port-8765 session mount before startup; do not reuse the old port-8000 URL |
