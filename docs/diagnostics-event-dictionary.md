...
| `pipeline.run.unexpected_failure` | ERROR | `event`, `outcome`, `reference_id`, `run_id`, `operation`, `exception_type`, `traceback` | `user_id` | Yes, redacted |
...
| `pipeline.run.queued` | INFO | `event`, `outcome`, `reference_id`, `run_id`, `pipeline_id`, `attempt`, `operation`, `stage` | `provider`, `user_id` | No |
| `pipeline.run.completed` | INFO | `event`, `outcome`, `reference_id`, `run_id`, `pipeline_id`, `attempt`, `operation`, `stage`, `provider`, `duration_ms`, `data_impact` | `user_id` | No |
| `pipeline.run.failed` | ERROR or WARNING when uncertain | `event`, `outcome`, `error_code`, `reference_id`, `run_id`, `pipeline_id`, `attempt`, `operation`, `stage`, `provider`, `duration_ms`, `retryable`, `data_impact`, `cause` | `provider_correlation_id`, `http_status`, `sqlstate`, `exception_type`, `user_id` | No |
