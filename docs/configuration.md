...
| Area | Variables |
| --- | --- |
| Application and database | `APP_ENV`, `APP_NAME`, `CUSTOM_THEME_ENABLED`, `PUBLIC_BASE_URL`, `DATABASE_URL` |
| Application secrets and token policy | `JWT_SECRET`, `SESSION_PEPPER`, `CSRF_SECRET`, `API_TOKEN_ENCRYPTION_KEYS`, `API_TOKEN_ACTIVE_KEY_ID`, `API_TOKEN_MAX_WRAPS_PER_KEY`, `JWT_ISSUER`, `JWT_AUDIENCE` |
| Authentication and browser policy | `AUTHENTICATION_MODE`, `PASSWORD_ONLY_PRODUCTION_RISK_ACCEPTED`, `TRUSTED_IDENTITY_HEADER`, `ACCESS_TOKEN_MINUTES`, `REFRESH_TOKEN_HOURS`, `SESSION_IDLE_MINUTES`, `COOKIE_SECURE`, `COOKIE_PATH`, `HSTS_INCLUDE_SUBDOMAINS`, `TRUSTED_PROXY_IPS`, `ALLOWED_EMAIL_DOMAINS` |
...
`app/config.py`. `EMAIL_FROM` also accepts the legacy `SMTP_FROM_EMAIL` name, and the two directory
aliases documented below remain supported for existing deployments.
