...
def test_login_page_document(access_app) -> None:
    ...
    assert_html_contains(
        response,
        'href="/app-assets/hedron-desktop.css?v=240926.1&amp;shell=5"',
    )
    assert "/assets/theme.css" not in response.body
    assert "/app-assets/data-mover-components.css" not in response.body
    assert_html_contains(response, 'src="/assets/app.js?v=240926.1&amp;ui=7"')
    assert response.body.count('src="/assets/app.js?v=240926.1&amp;ui=7"') == 1
    ...
    assert_html_contains(response, "Version 240926.1")
...
def test_live_production_shell_reports_effective_runtime_mode(color_mode) -> None:
    ...
    assert "Version 240926.1" in rendered
